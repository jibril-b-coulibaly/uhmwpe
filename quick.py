...
author: Jibril B. Coulibaly (jibril.coulibaly at gmail.com)
...
def tangent(sig_t, f_t, fvA_tplusdt, fvB_tplusdt,
            erate, dt, muA, muB_tplusdt, muC, k, q, lamL):
...
# Uniaxial integration of the constitutive behavior
# Return stress and deformation gradient at selected timesteps
def main(erate, etrue, dt, nsave):
  ...
  for ipath in range(npath):
    erate = np.abs(erate) * np.sign(deeng[ipath])
    nstep = np.ceil(deeng[ipath]/(erate*dt)).astype('int')
    nevery = np.round(np.linspace(t+1,t+nstep,nsave))
    # Warning: no check if nstep > nsave
    for istep in range(nstep):
      t +=1 # Current timestep
      flag = (nevery == t)
      if (np.any(flag)):
          idx = ipath*nsave + np.where(flag)[0][0]
      ...
      ds_df = tangent(sig, f, fvA, fvB, erate, dt, muA, muB, muC, kap, q, lamL)
